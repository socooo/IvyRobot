...
# 多线程分布式异步处理模型
#############################
class async_TCP_Server_Factory():
    def create_async_TCP_Server(self, type):
        map_ = {
            # 'asyn_TCP_Server_001': eval('asyn_TCP_Server_001()'),
            # 'asyn_TCP_Server_002': eval('asyn_TCP_Server_002()')

            # 'asyn_TCP_Server_001': asyn_TCP_Server_001(),
            # 'asyn_TCP_Server_002': asyn_TCP_Server_002()
...
class async_TCP_Server():
    def parm_mas_add(self):
        pass
    def tcpServer(self, server_name):
        pass
    def tcpClient(self):
        pass

#处理parm_mas_add
class asyn_TCP_Server_001(async_TCP_Server):
    # 业务逻辑
    def parm_mas_add(self, key, random_id):
        try:
            parm_mas=Parm_mas()
    ...
    def tcpServer(self):
        ...
        # port = int(utility.get_myibs_ini(server_name, "port"))
        # timeout = int(utility.get_myibs_ini(server_name, "timeout"))
        # msg_length = int(utility.get_myibs_ini(server_name, "msg_length"))
        # con_connection = int(utility.get_myibs_ini(server_name, "con_connection"))
        ...
        try:
            ...
            while True:
                ...
                rep = self.parm_mas_add(req, random.randint(1, 10))
...
class asyn_TCP_Server_002(async_TCP_Server):
    # 业务逻辑
    def parm_mas_add(self, key, random_id):
        try:
            parm_mas=Parm_mas()
    ...
    def tcpServer(self):
        ...
        port = int(utility.get_myibs_ini("asyn_TCP_server_002", "port"))
        timeout = int(utility.get_myibs_ini("asyn_TCP_server_002", "timeout"))
        msg_length = int(utility.get_myibs_ini("asyn_TCP_server_002", "msg_length"))
        con_connection = int(utility.get_myibs_ini("asyn_TCP_server_002", "con_connection"))
